...
2. Go to **Admin → Festival Year & Days** and create this year (e.g. 2026), then
   add each festival day (Chaturthi day 1, day 2, ... Visarjan day) with its date.
   Everything else (collections, expenses, programs, annadanam) hangs off these
   days, so set this up first.
3. Use **Collections** and **Expenses** to enter donations/spending per day —
   these screens are built for quick entry during a rush (numeric keypad inputs).
4. Use **Programs**, **Sponsors**, **Annadanam Sponsors**, and **Velam Paata
   Items** to fill in the rest.
5. For velam paata: add each item with a starting price and optional photo, then
